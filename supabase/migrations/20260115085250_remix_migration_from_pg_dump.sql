...
CREATE POLICY "Users can create orders" ON public.orders FOR INSERT WITH CHECK (((auth.uid() = user_id) OR ((user_id IS NULL) AND (guest_email IS NOT NULL))));
...
CREATE POLICY "Users can insert order items" ON public.order_items FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.orders
  WHERE ((orders.id = order_items.order_id) AND ((orders.user_id = auth.uid()) OR ((orders.user_id IS NULL) AND (orders.guest_email IS NOT NULL)))))));
...
CREATE POLICY "Users can view their own order items" ON public.order_items FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.orders
  WHERE ((orders.id = order_items.order_id) AND ((orders.user_id = auth.uid()) OR ((auth.uid() IS NULL) AND (orders.user_id IS NULL)))))));
...
CREATE POLICY "Users can view their own orders" ON public.orders FOR SELECT USING (((auth.uid() = user_id) OR ((auth.uid() IS NULL) AND (user_id IS NULL))));
